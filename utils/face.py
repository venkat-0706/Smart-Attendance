...
# === Professional, Deployment-Ready Paths ===
...
def load_image_safe(path):
    try:
        ...
        if img is None:
            print(f"[ERROR] Could not load image {path.name}")
            return None
    ...
    for (top,right,bottom,left), face_encoding in zip(face_locations, face_encodings):
        ...
        try:
            ...
            if not is_real_face(lbp_features):
                cv2.putText(frame, "Spoof Detected", (left_exp, top_exp-10),
            ...
            if matches[best_index] and distances[best_index] < 0.45:
                ...
                if name not in detection_start_time:
                    ...
                    if duration >= 2 and name not in attendance_marked:
                        ...
                        img_path = CAPTURE_SAVE_DIR / f"{name}_{timestamp}.jpg"
